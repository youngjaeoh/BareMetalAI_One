                ...
                if (header_idx + 3 + length < SPI_BUFFER_SIZE) {
                    if (check_packet_checksum((uint8_t*)recvbuf, header_idx, length)) {
                        ...
                        if (length > 0) {
                            ...
                            } else if (strstr(received_data, "airconditioner on") != NULL) {
                                ac_status = true;
                                ESP_LOGI(TAG, "Air Conditioner turned ON");
                                otInstance *ot_instance = esp_openthread_get_instance();
                                if (ot_instance) {
                                    otDeviceRole role = otThreadGetDeviceRole(ot_instance);
                                    ESP_LOGI(TAG, "Current Thread role: %d", role);
                            ...
                            } else if (strstr(received_data, "airconditioner off") != NULL) {
                                ac_status = false;
                                ESP_LOGI(TAG, "Air Conditioner turned OFF");
                                otInstance *ot_instance = esp_openthread_get_instance();
                                if (ot_instance) {
                                    otDeviceRole role = otThreadGetDeviceRole(ot_instance);
                                    ESP_LOGI(TAG, "Current Thread role: %d", role);
                            ...
                            } else if (strstr(received_data, "tv on") != NULL) {
                                tv_status = true;
                                ESP_LOGI(TAG, "TV turned ON");
                                otInstance *ot_instance = esp_openthread_get_instance();
                                if (ot_instance) {
                                    otDeviceRole role = otThreadGetDeviceRole(ot_instance);
                                    ESP_LOGI(TAG, "Current Thread role: %d", role);
                            ...
                            } else if (strstr(received_data, "tv off") != NULL) {
                                tv_status = false;
                                ESP_LOGI(TAG, "TV turned OFF");
                                otInstance *ot_instance = esp_openthread_get_instance();
                                if (ot_instance) {
                                    otDeviceRole role = otThreadGetDeviceRole(ot_instance);
                                    ESP_LOGI(TAG, "Current Thread role: %d", role);
